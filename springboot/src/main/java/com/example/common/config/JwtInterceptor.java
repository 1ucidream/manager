...
public class JwtInterceptor implements HandlerInterceptor {
    ...
    /**
     * 大概就是, 从token里面取用户名, 通过用户名查到对应密码, 再验证
     */
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        try {
            // 解析token获取存储的数据
            String userRole = JWT.decode(token).getAudience().get(0); // decode解码
            String userId = userRole.split("-")[0]; // 字符串的前半段
        ...
        } catch (Exception e) { //如果没查到, 那么说明token是非法的
